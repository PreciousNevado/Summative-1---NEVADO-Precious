...
for key, value in jsondata.items():
    value = str(value)
    value = value.upper()
    for unnecessary_text in unnecessary_text_list:
        if unnecessary_text in value:
            value = value.replace(unnecessary_text,"")
    for punctuation in unnecessary_punctuation: 
...
for word in value.split(" "): 
    if len(word)>=3: 
        ...
        wordcount_dictionary.setdefault(word, count)
...
for key, value in wordcount_dictionary.items():
        final_wordlist_of_tuples tuple(key, value)
...
gold_available = 18000
max_gold_F = gold_available // 1000  
max_gold_R = gold_available // 600 
max_gold_C = gold_available // 750 

lumber_available = 12000
max_lumber_F = lumber_available//500
max_lumber_R = lumber_available//750
max_lumber_C = lumber_available//300

food_available = 1500
max_food_F = food_available//50
max_food_R = food_available//45
max_food_C = food_available//75

max_possible_F = min(max_gold_F, max_lumber_F, max_food_F)
max_possible_R = min(max_gold_R, max_lumber_R, max_food_R)
max_possible_C = min(max_gold_C, max_lumber_C, max_food_C)


power_of_the_rading_party_list = []
for f in range(max_possible_F):
    for r in range(max_possible_R): 
        for c in range(max_possible_C):
            power_of_the_rading_party = (10*f) + (12*r) + (16*c)
            if 1000*f + 600*r + 750*c <= gold_available and 500*f + 750*r + 300*c <=lumber_available and 50*f + 45*r + 75*c <= food_available:
                power_of_the_rading_party_list.append(power_of_the_rading_party)
                if power_of_the_rading_party == max(power_of_the_rading_party_list):
                    optimal_Z = power_of_the_rading_party
                    optimal_F = f
                    optimal_R = r
                    optimal_C = c 
...
def Collatz(start_number):
    ...
    sequence_sum = start_number
    
    while number != 1: 
        if number % 2 == 0: 
            quotient = int(number/2)
            collatz_sequence.append(quotient)
            sequence_sum += quotient 
        else: 
            quotient = int((3 * number) + 1)
            collatz_sequence.append(quotient)
            sequence_sum += quotient 
        number = quotient
...
def Collatz_winner(number_list):
    ...
    for list in number_list:
        length_of_all_lists.append(len(str(list)))
        if len(str(list)) == max(length_of_all_lists):
            return(len(str(list)))
        else: 
            "NO WINNER"
